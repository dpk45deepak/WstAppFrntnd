...
const DriversManagementPage: React.FC = () => {
  ...
  const getAvailabilityBadge = (available: boolean) => {
    return available ? (
      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-teal-500 to-blue-500 text-white shadow-lg shadow-teal-500/30 animate-pulse">
        <MapPin className="w-3 h-3 mr-1.5 animate-bounce" />
        Available Now
      </span>
    ) : (
      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-rose-500 to-rose-600 text-white">
  ...
  const getRatingStars = (rating?: number) => {
    ...
    return (
      <div className="flex items-center group">
        ...
        <span className="ml-1 text-xs font-medium bg-gradient-to-r from-yellow-500 to-amber-500 bg-clip-text text-transparent">
          ({rating.toFixed(1)})
        </span>
      </div>
    );
      ...
      <div ref={statsRef} className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          >
            <div className="relative">
              <div
                className={`absolute -right-4 -top-4 w-20 h-20 rounded-full ${stat.gradient} ${stat.color} opacity-10 group-hover:scale-125 transition-transform duration-500`}
              ></div>
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`p-3 rounded-2xl ${stat.gradient} ${stat.color} text-white shadow-lg group-hover:rotate-12 transition-transform duration-500`}
                >
                  {stat.icon}
                </div>
              </div>
                ...
                <div className="pt-6 border-t border-gray-100">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Button
                        size="sm"
                        // variant={driver.availability ? "danger" : "gradient"}
                        // gradient={driver.availability ? undefined : "teal-blue"}
                        onClick={() =>
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          size="sm"
                          // variant={driver.availability ? "outline" : "gradient"}
                          // gradient={
